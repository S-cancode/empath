...
15a. **Post-approval: the review affordance in the shipped binary.** The `store-review` build carries `EXPO_PUBLIC_REVIEW_MODE=true`, so the submitted (and therefore public) binary shows a "App Review Access" row in Profile and calls `POST /review/demo-conversation` once per launch. This is fully server-gated (needs `REVIEW_MODE=true` + a durable grant from the secret code) — a normal user cannot reach the demo — but it is a visible affordance and a per-launch request. Choose one before/after approval: (a) **set `REVIEW_MODE=false` on the backend once review is complete** — the row stays but is inert and the endpoint 404s (simplest); or (b) ship the actual public release from the plain `production` profile (no review row) and reserve `store-review` for the review submission only. Owner: Shivan.
...
16a. **Register the Apple server-to-server notification endpoint.** The backend exposes `POST /auth/apple/notifications` (public; authenticity is the Apple JWT signature). Enter its production URL — `https://empath-production.up.railway.app/auth/apple/notifications` — as the **Server-to-Server Notification Endpoint** for the App ID / Services ID in the Apple Developer portal, so Apple sends `consent-revoked`/`account-delete` events. On a verified event the backend invalidates that account's sessions/sockets/matching and clears the stored Apple token (it does NOT auto-erase legally retained data). The client additionally polls Apple credential state on resume as defence in depth. Owner: Shivan. Done when: endpoint registered and a test "Stop Using Apple ID" invalidates the Empath session.
